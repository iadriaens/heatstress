...
class LT_connect(object):
    ...
    def __init__(self, pgres_host, pgres_port, db, ssh, keybased, ssh_user, ssh_host, ssh_pwd, psql_user, psql_pass, ssh_port, ssh_pkey):
        '''
    ...
    def tables(self, schema):
        ...
        engine = create_engine(
            f'postgresql://{self.psql_user}:{self.psql_pass}@{self.pgres_host}:{self.local_port}/{self.db}')
    ...
    def create_db(self, db):
        ...
        # of functions can be found at https://docs.sqlalchemy.org/en/14/
        engine = create_engine(
            f'postgresql://{self.psql_user}:{self.psql_pass}@{self.pgres_host}:{self.local_port}/{db}')
